...
export default {
  ...
  methods: {
    ...
    init() {
      //查询医院信息
      hospitalApi.getHospInfo(this.hoscode).then((response) => {
        this.hospital = response.data.hospital;
        this.bookingRule = response.data.bookingRule;
      });
      //查询相关科室信息
      hospitalApi.getDepartment(this.hoscode).then((response) => {
    ...
    schedule(depcode) {
      ...
      //静态方式访问,不是动态路由,会去访问schedule.vue或者schedule下面的index.vue页面
